...
function myrepeat(s: string, count: number): string {
    if (count < 1) return "";
    let result: string = "",
        pattern: string = s.valueOf();
    while (count > 0) {
...
function get_base_relative_path(args: any): string {
...
function get_relative_path(args: any): string {
...
interface ShlomifJQtreeRet {
    id: number;
    label: string;
    children?: ShlomifJQtreeRet[];
}

function calc_jqtree_data_from_html_w_nav_menu_json(
    args: any,
): ShlomifJQtreeRet {
    const rel_path = args.rel_path;
    ...
    _recurse = function (sub_tree: any): ShlomifJQtreeRet | ShlomifJQtreeRet[] {
        ...
        const label =
            sub_tree["url"] === rel_path
                ? "<b>" + sub_tree["text"] + "</b>"
                : '<a href="' +
                  escape_html(_get_rel(sub_tree["url"])) +
                  '"' +
                  title_attr +
                  ">" +
                  sub_tree["text"] +
