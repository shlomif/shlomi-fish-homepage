...
s#<(cc_by_licence_section|docbook_formats_w_base|lightning_talks_list|mailto_link_to_self|qp-lect|rellink|shlomif_docbook_doc_text)\s([^>]*?)/>#
    my ($tag, $args) = ($1, ($2 // ''));
    $tag =~ tr/-/_/;
...
my $re =
qr#(?:intro|perl_for_newbies_entry|note|modern_perl_entry|beginning_perl_entry|cpan_dist|pdoc|pdoc_f|licence_sect|links_sect|see_also|h[234]_section)#;
