...
$(FORTS_EPUB_DEST): $(FORTUNES_XHTMLS) $(FORTUNES_XHTMLS_TOCS) $(FORTS_EPUB_ALL_COVERS)
	export EBOOKMAKER="$$PWD/lib/ebookmaker/ebookmaker" PYTHONPATH="$${PYTHONPATH}}:$(SCREENPLAY_COMMON_INC_DIR)" DFN="$(PWD)/$(FORTS_EPUB_DEST)" JSONFN="book.json" ; cd $(FORTUNES_XHTMLS_DIR) && $${EBOOKMAKER} --output "$$DFN" "$$JSONFN" && python3 $(PWD)/bin/amend_epub.py
