/* CSEE.H - a library of ANSI C graphical functions.
	...
	To get a help document, please send 3 NIS (1 dollar) to this addres:
		Shlomi Fish
		4 Drezner St.
		Tel-Aviv 69497
...
void cirrectan ( int StartX, int StartY, int EndX, int EndY, float CirPart)
{
	...
	{
		cprintf("Unable to make function \"cirrectan()\"");
	}
	else
	{
	int p, DelX;
...
int draw(char * s, int ang)
{
	...
	if (strlen(s) > 300)
	{
		cprintf("The size of the input string for the \"draw()\" function is bigger than 300.\nPlease decrease the size");
		goto end;
	}
	strcpy(r, s);
	func = r[0];
