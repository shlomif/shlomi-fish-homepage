...
my $doc = $parser->load_html( location => shift(@ARGV) );
...
my ($toc) = $doc->findnodes("//table[\@id='toc']");
remove($toc);

{
    my ($el) = $doc->findnodes("//div[\@class='printfooter']");
...
foreach my $el ( $doc->findnodes("//script") )
{
    remove($el);
}
...
{
    foreach my $el ( $doc->findnodes("//*[\@$attr]") )
    {
        $el->removeAttribute($attr);
    }
}

foreach my $a_el ( $doc->findnodes("//p/a[\@id]") )
{
...
foreach my $a_el ( $doc->findnodes("//a[\@href]") )
{
    my $href = $a_el->getAttribute("href");
    if ( $href =~ m{\A/} )
    {
        $a_el->setAttribute( "href",
            "http://teachingopensource.org/index.php/" );
    }
}

foreach my $span_a_el ( $doc->findnodes("//span/a[text() = 'edit']") )
{
    remove( $span_a_el->parentNode() );
}

my $text = $doc->toString();

$text =~ s{(<ol>|</ol>|<ul>|</ul>|<li>|</li>|<p>|</p>)}{\n$1\n}g;
